...
Opulentus answers phone calls, holds provider API keys, and can place outbound calls
...
2. **Email** — contact Opulentus with `SECURITY` in the subject.
...
**There is no released version yet.** Opulentus is at Milestone 0 and is not ready for
...
- **Deployment choices the operator makes.** Exposing the port to the internet,
  ...
  vulnerabilities in Opulentus. The documentation states the supported paths: VPN,
  Tailscale, or a reverse proxy with HTTPS.
- **Vulnerabilities in third-party providers.** Report those to the provider. Tell us
  as well if Opulentus's use of them makes the impact worse.
...
- **Configure geographic dialling permissions at your telephony provider** and disable
  ...
  against toll fraud, it is free, and it works even if Opulentus itself has a bug.
