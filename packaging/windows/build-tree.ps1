...
Invoke-WebRequest -Uri "https://github.com/astral-sh/python-build-standalone/releases/download/$pythonBuild/cpython-$pythonVersion+$pythonBuild-x86_64-pc-windows-msvc-install_only_stripped.tar.gz" -OutFile "$env:TEMP\\opulentus-python.tar.gz"
tar -xzf "$env:TEMP\\opulentus-python.tar.gz" -C $app

Invoke-WebRequest -Uri "https://nodejs.org/dist/v$nodeVersion/node-v$nodeVersion-win-x64.zip" -OutFile "$env:TEMP\\opulentus-node.zip"
Expand-Archive -LiteralPath "$env:TEMP\\opulentus-node.zip" -DestinationPath "$env:TEMP\\opulentus-node" -Force
Move-Item -LiteralPath "$env:TEMP\\opulentus-node\\node-v$nodeVersion-win-x64" -Destination (Join-Path $app "node")
...
Copy-Item -Force (Join-Path $PSScriptRoot "opulentus.env") (Join-Path $app ".env")
...
Set-Content -NoNewline -Encoding ascii -Path (Join-Path $app "version.txt") $env:OPULENTUS_VERSION
...
Invoke-WebRequest -Uri "https://github.com/winsw/winsw/releases/download/v2.12.0/WinSW-x64.exe" -OutFile (Join-Path $app "OpulentusService.exe")
Copy-Item -Force (Join-Path $app "OpulentusService.exe") (Join-Path $app "OpulentusWebService.exe")
Copy-Item -Force (Join-Path $PSScriptRoot "opulentus-service.xml") (Join-Path $app "OpulentusService.xml")
Copy-Item -Force (Join-Path $PSScriptRoot "opulentus-web-service.xml") (Join-Path $app "OpulentusWebService.xml")
